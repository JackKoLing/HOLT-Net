...
class Instance_Loss(nn.Module):
    # 计算实例分支的loss
    def __init__(self, dataset="smoker-det"):
        super(Instance_Loss,self).__init__()
        self.focal_loss = FocalLoss()
        self.bce_loss = nn.BCELoss()
        self.dataset = dataset

        with open("datasets/smoker_det/annotations/smoker-det_verb_count.json", "r") as file:
            verb_count = json.load(file)

        verb_count = verb_count + verb_count # [400, 400, 400, 400, 400, 400]
    ...
    def forward(self, act_classifications, obj_classifications, obj_regressions, anchors, inst_annotations,  **kwargs):
        ...
        if imgs is not None:
            ...
            obj_list = kwargs.get('obj_list', None)
            out = postprocess(imgs.detach(),
                              torch.stack([anchors[0]] * imgs.shape[0], 0).detach(), obj_regressions.detach(), obj_classifications.detach(),
                              regressBoxes, clipBoxes,
                              0.5, 0.3)
            imgs = imgs.permute(0, 2, 3, 1).cpu().numpy()
            imgs = ((imgs * [0.229, 0.224, 0.225] + [0.485, 0.456, 0.406]) * 255).astype(np.uint8)
