...
class Regressor(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for feat, bn_list in zip(inputs, self.bn_list):
            ...
            feat = feat.permute(0, 2, 3, 1)  # (batch_size, height, width, num_anchor*4)
...
class Classifier(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for feat, bn_list in zip(inputs, self.bn_list):
            ...
            feat = feat.permute(0, 2, 3, 1)  # (batch_size, height, width, num_anchors*num_classes)
            feat = feat.contiguous().view(feat.shape[0], feat.shape[1], feat.shape[2], self.num_anchors,
                                          self.num_classes)  # (batch_size, height, width, num_anchors, num_classes)
            feat = feat.contiguous().view(feat.shape[0], -1, self.num_classes)  # (batch_size, h*w*num_anchor, num_classes)

            feats.append(feat)

        feats = torch.cat(feats, dim=1)  # (batch_size, h*w*num_anchor*feat_num, num_classes)
        feats = feats.sigmoid()
